...
from nns.model.DISTN import Model
from nns.spv.loss import *
from nns.spv.base_spv import BaseSupervisor
import pywt


class DISTNSupervisor(BaseSupervisor):
    def __init__(self, **kwargs):
        super(DISTNSupervisor, self).__init__(**kwargs)
    ...
    def _get_model(self):
        ...
        periods = self._get_train_periods()
        model = Model(periods=periods, device=self.device, **self._model_kwargs)
        return model
    ...
    def _get_train_stats(self):
        self.logger.info('Calculating train stats for normalization...')
        num_train_sample = len(self.data['train_data'])
    ...
    def _gen_run_id(self):
        ...
        num_nodes = self._model_kwargs.get('num_nodes', 207)
        ...
        if model_id == 'DISTN_Simple':
            cross_linear = 1 if self._model_kwargs.get('cross_linear', False) else 0
            model_settings = f'_cl{cross_linear}'
        elif model_id == 'DISTN_SegV2':
            norm_type = self._model_kwargs.get('norm_type', 'global')
            d_node_embed = self._model_kwargs.get('d_node_embed', 64)
            num_neighs = self._model_kwargs.get('num_neighs', 3)
            d_lf_embed = self._model_kwargs.get('d_lf_embed', 128)
            d_hf_embed = self._model_kwargs.get('d_hf_embed', 64)
            num_layers = self._model_kwargs.get('num_layers', 0)
            dropout = self._model_kwargs.get('dropout', 0.2)
            model_settings = f'_{norm_type}_dne{d_node_embed}_nn{num_neighs}_dle{d_lf_embed}_dhe{d_hf_embed}_nl{num_layers}_do{dropout}'
        elif model_id == 'DISTN_SegV3':
            d_node_embed = self._model_kwargs.get('d_node_embed', 64)
            num_neighs = self._model_kwargs.get('num_neighs', 3)
            d_lf_embed = self._model_kwargs.get('d_lf_embed', 128)
            d_hf_embed = self._model_kwargs.get('d_hf_embed', 64)
            seg_len = self._model_kwargs.get('seg_len', 12)
            num_layers = self._model_kwargs.get('num_layers', 0)
            dropout = self._model_kwargs.get('dropout', 0.2)
            model_settings = f'_dne{d_node_embed}_nn{num_neighs}_dle{d_lf_embed}_sgl{seg_len}_dhe{d_hf_embed}_nl{num_layers}_do{dropout}'
        else:
            seg_len = self._model_kwargs.get('seg_len', 12)
            dropout = self._model_kwargs.get('dropout', 0.2)
            model_settings = f'_sl{seg_len}_do{dropout}'
        ...
        run_id = f'{data_name}_sl{seq_len}_hl{horizon}_{model_id}_bs{batch_size}_{wavelet}_l{dwt_level}' \
                 f'_N{num_nodes}{model_settings}{loss_settings}_{run_time}'
    ...
    def train_epoch(self, epoch):
        ...
        for batch_idx, (x, y, x_mark, y_mark) in enumerate(self.train_loader):
            ...
            y_pred, _, _ = self.model(xl, xh, self.stats_xl_diff, self.stats_xh)
    ...
    def test(self, test_loader=None, message=None, plot_id=None, extra_tb_hparams=None):
        ...
        with torch.no_grad():
            for batch_idx, (x, y, x_mark, y_mark) in enumerate(test_loader):
                xl, xh, yl, yh, y, x_mark, y_mark = self.prepare_data(x, y, x_mark, y_mark, dwt_y=True)
                y_pred, _, _ = self.model(xl, xh, self.stats_xl_diff, self.stats_xh)
        ...
        if 'Fused' in self._kwargs.get('model_id', 'DISTN'):
            self.logger.info(f"Fusion Params {self.model.fusion_param.item():.3f}")
        ...
        if self._save_tb:
            ...
            if extra_tb_hparams is not None:
                hparams_dict.update(extra_tb_hparams)
